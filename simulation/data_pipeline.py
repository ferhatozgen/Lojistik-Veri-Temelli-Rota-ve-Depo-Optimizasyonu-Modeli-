...
def get_weather_for_date(dt, today_date):
    ...
    if dt.date() < today_date:
        # Geçmiş boşlukları doldururken Mayıs ayı Edirne normları (%25 Yağmur, %75 Açık)
        return 1 if random.random() < 0.25 else 0
    else:
        # Geleceğe ait günleri simüle ederken forecast tahmini (%20 Yağmur, %80 Açık)
...
def bridge_data_gap_system(target_date_str):
    ...
    print(f"⚠ Veride Boşluk/Gelecek Gün Talebi Saptandı! Son Kayıt: {last_recorded_date} -> Hedef: {target_date}")
    print(f"⚙ Aradaki günler demand_engine kurallarıyla tıkır tıkır dolduruluyor...")
    ...
    # Döngünün başlayacağı gün: Son kayıtlı günün 1 gün sonrası
    ...
    while current_processing_date <= target_date:
        for hour in range(24):
            ...
            # Akıllı hava durumu ve hafta sonu kontrolü
            ...
            for district_name, profile in DISTRICT_PROFILES.items():
                ...
                # Saatlik kümülatif demand kaydı
                ...
                # Harita ve Rota için tekil koordinatlı sipariş kayıtları
    ...
    # 2. Üretilen Verileri Mevcut CSV Dosyalarının Altına Ekleme (Append Modu)
    if new_demand_rows:
        ...
        print(f"✅ Başarılı! Dosyalar güncellendi. Sistem {target_date_str} tarihine hazır hale getirildi.")


if __name__ == "__main__":
    # Test çalıştırması: 28 Mayıs miladına göre 30 Mayıs'a kadar olan akışı tetikleyelim
