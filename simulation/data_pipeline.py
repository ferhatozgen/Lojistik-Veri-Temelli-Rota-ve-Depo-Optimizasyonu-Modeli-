...
def bridge_data_gap_system(target_date_str, today_date_str='2026-05-28'):
    ...
    if not os.path.exists(demand_path) or not os.path.exists(orders_path):
        print("Hata: Başlangıç veri setleri bulunamadı!")
        return
    ...
    if target_date <= last_recorded_date:
        print(f"Talep edilen tarih ({target_date_str}) mevcut. Doğrudan yükleniyor.")
        return

    print(f"⚠ Boşluk saptandı: {last_recorded_date} -> {target_date}")
    ...
    while current_processing_date <= target_date:
        for hour in range(24):
            ...
            for district_name, profile in DISTRICT_PROFILES.items():
                ...
                # hourly_demand.csv — tüm feature'lar burada kalıyor (LSTM için gerekli)
    ...
    if new_demand_rows:
        ...
        print(f"✅ Tamamlandı. Sistem {target_date_str} tarihine hazır.")
